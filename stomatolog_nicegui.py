...
try:
    from core.log_utils import log, LOG_DIR, STDOUT_LOG_FILE, read_tail
except Exception:
    def log(message: str):
...
def main():
    ...
    async def healthcheck():
        """Basic health endpoint for the external frontend."""
        cfg = ConfigManager() if ConfigManager else None
        backend = None
        model = None
        device = None
        if cfg:
            backend = cfg.get("transcriber_backend", "")
            model = cfg.get("transcriber_model", "")
    ...
    async def debug_logs(request: Request):
        """Returns last lines from app and stdout logs for easy copy/paste."""
        try:
            tail = int(request.query_params.get("tail", "200"))
        except ValueError:
            tail = 200
        tail = max(10, min(tail, 2000))
        app_lines = read_tail(LOG_DIR / "app.log", tail)
            ...
            def _open_pages_existing():
                try:
                    ...
                    if os.environ.get("WYWIAD_OPEN_LANDING") == "1":
                        webbrowser.open("https://guziczak.github.io/wizyta/", new=1, autoraise=True)
                        time.sleep(0.5)
                    if auto_open:
